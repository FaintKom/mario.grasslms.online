...
import { brandIcon, hydrateBrandIcons } from "./brand-icons.js";
...
export class MiniOS {
  ...
  _buildWindowDom(winId, def) {
    ...
    const brand = brandIcon(def.id, 16);
    el.innerHTML = `
      <header class="os-window-titlebar" data-region="titlebar">
        <div class="os-window-title">
          ${brand || `<span class="app-glyph" aria-hidden="true">${def.id.slice(0,1).toUpperCase()}</span>`}
          ${def.name}
        </div>
        <div class="os-window-controls">
          <button type="button" data-action="minimize" aria-label="Minimize ${def.name}" title="Minimize">
            <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden="true"><path d="M1 5h8" stroke="currentColor" stroke-width="1" fill="none"/></svg>
          </button>
          <button type="button" data-action="maximize" aria-label="Maximize ${def.name}" title="Maximize / restore">
            <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden="true"><rect x="1.5" y="1.5" width="7" height="7" stroke="currentColor" stroke-width="1" fill="none"/></svg>
          </button>
          <button type="button" data-action="close" aria-label="Close ${def.name}">
            <svg width="10" height="10" viewBox="0 0 10 10" aria-hidden="true"><path d="M1.5 1.5l7 7M8.5 1.5l-7 7" stroke="currentColor" stroke-width="1" fill="none"/></svg>
          </button>
        </div>
      </header>
  ...
  _addTaskbarEntry(winId, def) {
    ...
    const brand = brandIcon(def.id, 24);
    li.innerHTML = `
      <button type="button" class="taskbar-item" data-win-id="${winId}" data-app="${def.id}" aria-pressed="false" aria-label="${def.name}" title="${def.name}">
        ${brand || icon(def.iconName)}
        <span class="dot"></span>
      </button>
    `;
